...
# Generate encryption key from a password (in production, use env variable)
def get_encryption_key():
    password = b"your-biometric-encryption-key-change-in-production"
    salt = b"biometric-salt-change-in-production"
    kdf = PBKDF2HMAC(
...
def enroll_face(base64_image: str) -> str:
    ...
    try:
        ...
        encrypted_encoding = encrypt_data(encoding_json)
...
def verify_face(base64_image: str, stored_encrypted_encoding: str, tolerance: float = 0.6) -> dict:
    ...
    try:
        ...
        decrypted_json = decrypt_data(stored_encrypted_encoding)
